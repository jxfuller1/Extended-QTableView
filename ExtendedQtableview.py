...
from PyQt5.QtGui import QColor, QFont, QStandardItemModel, QWheelEvent, QKeyEvent

from PyQt5.QtWidgets import QApplication, QTableView, QVBoxLayout, QAbstractItemView, \
    QAbstractItemDelegate, QStyledItemDelegate, QPushButton, QWidget,QStyleOptionButton, QStyle, \
    QTableWidget, QHeaderView, QLabel, QLineEdit, QDialogButtonBox, QDialog, QTableWidgetItem, QComboBox, QFrame, \
    QCheckBox, QHBoxLayout, QSizePolicy, QCalendarWidget, QDateEdit, \
    QMenu, QAction, QMessageBox, QFileDialog
from PyQt5.QtCore import Qt, QAbstractTableModel, QEvent, QSize, QRect, QModelIndex, pyqtSignal, \
    QSortFilterProxyModel, QPoint, pyqtSlot, QCoreApplication, QTimer, QDate
...
class CustomTableView(QTableView):
    ...
    def __init__(self, model, columns_with_checkboxes: List[int] = None, checked_indexes_rows: Dict[int, List[int]] = None,
        ...
        # Apply style to hide the built in frame in order to ues custom one
    ...
    def sub_table_populate(self, sub_table_index: int, widget: QWidget):
        ...
        if self.sub_table_data is not None and len(self.model.table_data) == len(self.sub_table_data):
...
class ComboBox(QComboBox):
    ...
    def showPopup(self):
        ...
        # the following code is for doing a custom location of the combobox dropdown.... however there's issues with doing this
        # it will first very quickly blink to it's default location and then move to the position i give it, even when
        # disabling the combobox app animation (code for that is in the setup_table class).....
        # so i'm blocking this code out for now... but i may want it for the future
...
class ButtonHeaderView(QHeaderView):
    ...
    def onSectionCountChanged(self):
        ...
        for i in range(self.count()):
            ...
            button.view().setStyleSheet("background-color: lightgrey;")
    ...
    def set_combo_column_filter_items(self, column: int, button: QComboBox, alter_already_set_filter: bool = False, altered_value: str = None):
        ...
        if not alter_already_set_filter:
            if base_index == 4:
                ...
                # NO LONGER USING THIS COMMENTED OUT CODE, but keeping around just in case
                ...
                if column_values:
                    remove_blanks = [str(item) for item in column_values if item != ""]
...
class sub_TableWidget(QTableWidget):
    ...
    # NOTE THIS FUNCTION NOT BEING USED FOR ANYTHING, but keeping around just in case
    @pyqtSlot()
...
# custom qframe for tableview due to bug with widgets overlapping frame of tableview, this is also the parent of qtableview
# to create table use this class
class setup_table(QFrame):
    resizeSignal = pyqtSignal(QSize)

    def __init__(self, maintable_data: List[str] = None, maintable_headers: List[str] = None, columns_with_checkboxes: List[int] = None,
        ...
        """
        :param maintable_data:   Main table data needs to be a list of list for each row
        :param maintable_headers:   Main table headers, needs to be list of strings
        :param columns_with_checkboxes:  Main table checkboxes, list of column indexes
        :param checked_indexes_rows:   Main table rows that will be checkmarked with checkboxes; needs to be dictionary with key being column and value is a list of row indexes that will be checkmarked
        :param sub_table_data:   Sub table data if using expandable rows; needs to be list of a list of rows for each table.  sub_table_data will NEED a list for every row of the main table, even if the list is empty.
        :param editable_columns:  List of indexes for columns you want to be editable
        :param parent:   parent (not needed to pass as a parameter)
        :param datetime_columns:  List of indexes for columns you want to be to use datetime/datetime calendar
        :param footer:   True or False on whether you want a footer
        :param footer_values:  Accepts a dictionary.  keys = column indexes, values = a string that can be "total" or "sum".  "sum" adds up integers/float values to get total.  "total" will either get total rows or total checked checkboxes
        :param subtable_col_checkboxes: List of column indexes you want the sub tables to have checkboxes
        :param sub_table_headers_labels:  Header labels for sub tables, needs to be list of strings
        :param expandable_rows:  True or False on whether you want expansion rows that contain sub-tables
        :param add_mainrow_option:  True or False. Gives option to add rows to main table when right clicking headers
        :param del_mainrow_option:  True or False. Gives option to delete rows on main table when right clicking headers
        :param add_subrow_option:  True or False. Gives option to add rows to sub tables when right clicking headers
        :param del_subrow_option:  True or False. Gives option to delete rows for sub tables when right clicking headers
        :param subtable_datetime_columns:  List of indexes you want sub tables to have datetime columns/calender
        :param dblclick_edit_only:  True or False.  True will make it so that table can be edited by double clicking table if you want that rather that editing directly on columns in table
        :param use_sql:  True or False.  Enable to use SQL to populate tables
        :param sql_maintable_path:  String.  SQL path for main table to pull data from
        :param sql_maintable_name:  String.  SQL table name to pull data from
        :param sql_maintable_query:  String.  If you want a specific query you want returned for the main table from the SQL database.  NOTE: THIS HASNT BEEN TESTED
        :param sql_subtable_path:  String.  SQL path for sub table data
        :param sql_subtable_name:  String.  SQL table name to pull sub table data from
        :param sql_subtable_query:  String.  If you want a specific query to be returned for the sub table from the SQL datadate.  NOTE:  THIS HASNT BEEN TESTED
        """

        super(setup_table, self).__init__()

        # this code is for disabling the qcombobox default drop down animation... which causes a blinking effect if enabled
        # when you first open it... which i hate
    ...
    def activate_table(self):
        ...
        if self.maintable_data is not None:
        ...
        self.table_view = CustomTableView(self.model, self.columns_with_checkboxes, self.checked_indexes_rows, self.sub_table_data,
    ...
    def sql_subtable_addrow(self, maintable_index: int, subtable_values: List[str]):
        ...
        if self.use_sql:
            ...
            # CODE FOR TESTING, NOT BEING USED
            #allsub_tables = SQL_table.sql_tables(self.sql_subtable_path)
            #sub_df = SQL_table.sql_subtable_to_dataframe(self.sql_subtable_path, allsub_tables, self.sql_subtable_name,
            #                                           self.sub_table_headers_labels, self.sql_subtable_query)
            #print(sub_df)
    ...
    def loadnew_maintable_sql(self, maintable_name: str, maintable_sql_path: str, maintable_query: str = None,
        ...
        # resize due to display issues with vertical headers if there are no initial rows in the table
        self.resize(self.width() + 1, self.height())
        self.resize(self.width() - 1, self.height())

    def loadnew_maintable_list(self, maintable_data: List[str], keep_existing_filter=False):
        """
        :param maintable_data:  List of lists, where each list represents ROW data (not column data)
        :param keep_existing_filter: whether to keep existing filters if some filters are set, when loading a new table data
        """
        self.maintable_data = maintable_data
        self.clear_table(keep_filter=keep_existing_filter)
        self.update_table()
    ...
    def loadnew_columns_with_checkboxes(self, columns_with_checkboxes: Union[List[int], None]):
        ...
        if self.table_view is not None:
            ...
            # reload the filter comboboxes to portray changes to which columns are checkboxes
    ...
    def loadnew_footervalues(self, footer_values: Dict[int, str]):
        """
        Note: if you are using expandable rows, you will need to compensate by adding +1 to your indexes
        """
        self.footer_value = footer_values
    ...
    def loadnew_subtable_headers(self, subtable_headers: List[str]):
        self.sub_table_headers_labels = subtable_headers

        if self.table_view is not None:
            self.table_view.subtable_header_labels = subtable_headers

    def loadnew_subtable_col_checkboxes(self, subtable_col_checkboxes: List[int]):
        self.subtable_col_checkboxes = subtable_col_checkboxes

        if self.table_view is not None:
            self.table_view.subtable_col_checkboxes = subtable_col_checkboxes

    def update_add_mainrow_option(self, add_mainrow_option: bool):
        self.add_mainrow_option = add_mainrow_option

        if self.table_view is not None:
            self.table_view.add_mainrow_option = add_mainrow_option

    def update_del_mainrow_option(self, del_mainrow_option: bool):
        self.del_mainrow_option = del_mainrow_option

        if self.table_view is not None:
            self.table_view.del_mainrow_option = del_mainrow_option

    def update_add_subrow_option(self, add_subrow_option: bool):
        self.add_subrow_option = add_subrow_option

        if self.table_view is not None:
            self.table_view.add_subrow_option = add_subrow_option

    def update_del_subrow_option(self, del_subrow_option: bool):
        self.del_subrow_option = del_subrow_option

        if self.table_view is not None:
            self.table_view.del_subrow_option = del_subrow_option

    def loadnew_subtable_datetime(self, subtable_datetime_columns: List[int]):
        self.subtable_datetime_columns = subtable_datetime_columns

        if self.table_view is not None:
            self.table_view.subtable_datetime_columns = subtable_datetime_columns

    def update_dblclick_edit_only(self, dblclick_edit_only: bool):
        self.dblclick_edit_only = dblclick_edit_only

        if self.table_view is not None:
            self.table_view.dblclick_edit_only = dblclick_edit_only

            delegate = self.table_view.itemDelegate()
            delegate.dblclick_edit_only = dblclick_edit_only

    def useFooter(self, footer: bool):
        self.footer = footer

        if self.table_view is not None:
            self.table_view.footer_show = footer
...
if __name__ == "__main__":
    ...
    #viewer = Qmainwindow_function(app)
    #viewer.show()
    #sys.exit(app.exec_())

    print("if you want to use this not as a module in a program, create a QMainwindow function to initialize"
          "the setup_table QFrame")
